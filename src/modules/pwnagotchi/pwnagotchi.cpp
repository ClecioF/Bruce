...
void brucegotchi_setup() {
    ...
    initUi();
    state = STATE_INIT;
    Serial.println("Brucegotchi Inicializado");
}
...
void advertise(uint8_t channel) {
    ...
    if (result == ESP_ERR_WIFI_IF) {
        setMood(MOOD_BROKEN, "", "Erro: interface invalida", true);
        state = STATE_HALT;
    } else if (result == ESP_ERR_INVALID_ARG) {
        setMood(MOOD_BROKEN, "", "Erro: argumento invalido", true);
        state = STATE_HALT;
    } else if (result == ESP_ERR_NO_MEM) {
        setMood(MOOD_BROKEN, "", "Erro: memoria insuficiente", true);
        state = STATE_HALT;
    } else if (result != ESP_OK) {
        setMood(MOOD_BROKEN, "", "Erro: desconhecido", true);
        state = STATE_HALT;
    }
}
...
void brucegotchi_start() {
    ...
    while (true) {
        if (millis() - tmp < 2000 && !Deauth_done) {
            Deauth_done = true;
            drawMood("(-@_@)", "Preparando Sniper de Deauth");
        }
        if (millis() - tmp > (2000 + 1000 * _times) && Deauth_done && !pwgrid_done) {
            ...
            drawMood(shot ? "(<<_<<)" : "(>>_>>)", shot ? "Lasers Ativados! Derrubando" : "pew! pew! pew!");
            _times++;
            shot = !shot;
        }
        if (millis() - tmp > 12000 && pwgrid_done == false) {
            drawMood("(^__^)", "Bora Fazer Amigos!");
            _times = 0;
            pwgrid_done = true;
        }
        ...
        if (check(SelPress)) {
            ...
            String channel_status = use_all_channels ? "Todos Ch: ON" : "Todos Ch: OFF";

            options = {
                {"Buscar amigos", yield},
                {"Spam do Pwngrid", send_pwnagotchi_beacon_main},
