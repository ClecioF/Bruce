...
void drawTime() {
    ...
    int8_t m = sr / 60;
    int8_t s = sr % 60;
    char right_str[50] = "BAT 0%  UP 00:00:00"; // Alterado de UPS para BAT (Bateria)
    sprintf(right_str, "BAT %i%% UP %02d:%02d:%02d", getBattery(), h, m, s);
...
void drawFooterData(uint8_t friends_run, uint8_t friends_tot, String last_friend_name, signed int rssi) {
    ...
    String stats = "AMIG 0 (0)"; // Alterado de FRND (Friends) para AMIG (Amigos)
    if (friends_run > 0) {
...
void drawTopCanvas() {
    ...
    tft.setTextDatum(TL_DATUM);
    char buffer[32];
    sprintf(buffer, "CH %02d, HS %d", ch, num_HS); // Mantidos termos tecnicos globais (Channel / Handshake)
    // draw screen
    tft.drawPixel(0, 0, 0);
...
void drawBottomCanvas() {
    ...
    tft.drawString("SEM IA", display_w, canvas_bot_h + 5); // Alterado de NOT AI para SEM IA
