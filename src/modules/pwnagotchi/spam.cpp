...
void displaySpamStatus() {
    ...
    tft.println("PwnGrid Spam Rodando..."); // Traduzido para a tela
    ...
    while (spamRunning) {
        ...
        if (check(SelPress)) {
            dos_pwnd = !dos_pwnd;
            Serial.printf("DoScreen %s.\n", dos_pwnd ? "enabled" : "disabled"); // Mantido original
        }
        if (check(NextPress)) {
            change_identity = !change_identity;
            Serial.printf("Change Identity %s.\n", change_identity ? "enabled" : "disabled"); // Mantido original
        }
        ...
        if (!dos_pwnd) {
            tft.setCursor(0, 50);
            tft.printf("Rosto: \n%s                                                ", faces[current_face_index]); // Traduzido para a tela
            tft.setCursor(0, 80);
            tft.printf(
                "Nome:                  \n%s                                                ", // Traduzido para a tela
                names[current_name_index]
            );
        } else {
            tft.setCursor(0, 50);
            tft.printf("Rosto:\nNOPWND!■■■■■■■■■■■■■■■■■"); // Traduzido para a tela
            tft.setCursor(0, 80);
            tft.printf("Nome:\n■■■■■■■■■■■■■■■■■■■■■■"); // Traduzido para a tela
        }
        tft.setCursor(0, 110);
        tft.printf("Canal: %d  ", channels[current_channel_index]); // Traduzido para a tela
    ...
    displayWarning("Parando.."); // Traduzido para a tela
...
void loadFacesAndNames() {
    ...
    options = {
        {"Rostos padrao", [&]() { look_for_file = false; }}, // Menu traduzido
    };
    if (setupSdCard()) {
        look_for_file = true;
        options.push_back({"Cartao SD", [&]() { // Menu traduzido
                               fs = &SD;
                               look_for_file = true;
                           }});
    }
    if (checkLittleFsSizeNM()) {
        look_for_file = true;
        options.push_back({"Rostos LittleFS", [&]() { // Menu traduzido
                               fs = &LittleFS;
                               look_for_file = true;
                           }});
    }
    ...
    Default: // This is default pwngrid faces to spam, removing the necessity to have a configuration file
        ...
        faces[i++] = "(X‿‿X)";
        faces[i++] = "(u W u)";
        faces[i++] = "(CAPI)"; // Alterado para combinar com o tema Capivara
        faces[i++] = "(.)(.)";
        faces[i++] = "ლ(o_oლ)";
        faces[i++] = "(O﹏o)";
        faces[i++] = "(✖╭╮✖)";
        ...
        num_faces = i;
        i = 0;
        names[i++] = "meu nome eh... CapiPwn!"; // Dispositivo Batizado!
        names[i++] = "Projeto Bruce Customizado";
        names[i++] = "┌∩┐(◣_◢)┌∩┐";
        names[i++] = "(╯°□°)╯╭╮(XoX)";
        names[i++] = "PARE DEAUTH SKIDZ!";
        names[i++] = "System Breached oups";
        names[i++] = "Unauthorized  Access";
        names[i++] = "Security  Compromised.. reboot";
        names[i++] = "Aviso... CapiPwn na area"; // Dispositivo Batizado!
        names[i++] = "Erro Critico precisa reiniciar";
        names[i++] = "Sem Bateria";
        ...
        if (!file) {
            Serial.println("Failed to open pwngrid file for reading"); // Mantido original
            return;
        }
...
void send_pwnagotchi_beacon_main() {
    ...
    if (num_faces == 0 or num_names == 0) {
        displayTextLine("Sem arquivo config"); // Traduzido para a tela
